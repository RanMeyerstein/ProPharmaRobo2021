...
BOOL CPharmaRobot10Dlg::OnInitDialog()
{
	...
	m_EditDsnSQL.SetWindowTextW(L"DRIVER=SQL Server;SERVER=192.168.1.4;UID=sa;PWD=B1Admin;DATABASE=PIRYON;TABLE=Xitems");
...
BOOL CPharmaRobot10Dlg::InitiateYarpaSQL()
{
	...
	wchar_t StringSQL[2000];
	wchar_t  st[100];

	memset(StringSQL, 0, 200);

	if (ConnectedToYarpaSQL== FALSE)
	{

		GetSqlConnectionLine(StringSQL);
...
void CPharmaRobot10Dlg::GetSqlConnectionLine(wchar_t* line)
{
	ifstream sql_con_file("C:\\tmp\\ProPharmaSQLLine.txt", ios::binary);

	if (sql_con_file.fail()) {
		m_EditDsnSQL.GetWindowTextW(line, m_EditDsnSQL.GetWindowTextLengthW() + 1);
	}
	else
	{
		sql_con_file.seekg(0, std::ios::end);
		size_t file_size_in_byte = sql_con_file.tellg();
		std::vector<char> data; // used to store text data
		data.resize(file_size_in_byte);
		sql_con_file.seekg(0, std::ios::beg);
		sql_con_file.read(&data[0], file_size_in_byte);

		mbstowcs(line, &data[0], strlen(&data[0]));
		m_EditDsnSQL.SetWindowText(line);
	}
}
